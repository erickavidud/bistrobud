(() => {
  ...
  function showView(name){$$('.view').forEach(v=>v.classList.toggle('active',v.id===`view-${name}`));$$('.nav-btn').forEach(b=>b.classList.toggle('active',b.dataset.view===name));$('#mainNav').classList.remove('open');window.scrollTo({top:0,behavior:'smooth'});history.replaceState(null,'',`#${name}`)}
  ...
  function showPane(name){$$('.pane').forEach(p=>p.classList.toggle('active',p.id===`pane-${name}`));$$('.side-btn').forEach(b=>b.classList.toggle('active',b.dataset.pane===name));}
